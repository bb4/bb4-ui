...
import java.awt._
import com.barrybecker4.ui.legend.LegendEditBar
...
class ColoredLegendLine private[legend](var cmap: ColorMap) extends JPanel {
  override def paintComponent(g: Graphics): Unit = {
    ...
    val rat = (getWidth - 2 * ColoredLegendLine.MARGIN).toDouble / cmap.getValueRange
    for (i <- 1 until cmap.getNumValues) {
      val xstart = rat * (cmap.getValue(i - 1) - firstVal)
      val xstop = rat * (cmap.getValue(i) - firstVal)
      val paint = new GradientPaint(xstart.toFloat, 0.0f, cmap.getColor(i - 1), xstop.toFloat, 0.0f, cmap.getColor(i), true)
      g2.setPaint(paint)
